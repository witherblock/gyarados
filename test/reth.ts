...
describe("REthRateProvider & REthRateReceiver", function () {
  async function deployFixture() {
    ...
    const REthRateProvider = await ethers.getContractFactory(
      "REthRateProvider"
    );
    const rateProvider = await REthRateProvider.deploy(
      CHAIN_ID,
  ...
  describe("Basic Update Test", function () {
    it("Should set the right rate and update it at receiver", async function () {
      ...
      expect(rate.toString()).to.be.equal("1070999366517302681");
    });
  });
});
